 ...
 *
 * @author Tony Zhao
 * @version $Id: FinalAuditRejectHandler.java, v 0.1 2022-05-05 8:43 PM Tony Zhao Exp $$
 */
@Service
public class FinalAuditRejectHandler implements ActionHandler<PickupMessage, Integer> {

    private static final Logger logger = LoggerFactory.getLogger(FinalAuditRejectHandler.class);

    @Override
    public Integer process(ActionChain chain, ProcessContext<PickupMessage> context) {

        LoggerUtil.warn(logger, "FinalAuditRejectHandler handler executed");

        List<Integer> orderIds = results(context, DraftOrderExistHandler.class);
        PickupMessage pickupMessage = param(context);
        // 这里remark存的是reject reason，check ok.
